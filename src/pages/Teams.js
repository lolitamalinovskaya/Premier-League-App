...
const Teams = ({state, dispatch}) => {
    ...
    const onClick = (teamId) => {
        fetch('https://polar-shelf-59117.herokuapp.com/api/v1/user-favorite-teams', {
            method: "POST",
            headers: {"Authorization": `Bearer ${state.accessToken}`, "Content-Type": "application/json", "Accept": "application/json, text/plain, */*"},
            body: JSON.stringify({
        ...
        <section>
            ...
            <div className={"teams"}>
                {!state.isLoaded ? <Progress/> :
                    state.teams && state.teams.map((team) => <Team onClick={onClick} team={team} key={team.id} id={team.id}/>)}
            </div>
        </section>
    )
}
